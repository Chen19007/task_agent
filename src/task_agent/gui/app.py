...
class TaskAgentGUI:
    ...
    def __init__(self, config: Config):
        ...
        self.status_text: Optional[Union[int, str]] = None
        ...
        # 确保 UI 组件已初始化以满足类型检查
        assert self.chat_panel is not None
        ...
        for msg in messages:
            ...
            if role == "assistant":
                if think and think.strip():
                    self.chat_panel.add_collapsible_block(
                        "[思考]", think, collapsed=True
                    )
    ...
    def _load_fonts(self):
        ...
        if font_file:
            ...
            with dpg.font_registry():  # type: ignore
                # 加载中文字体并添加中文字符范围
                with dpg.font(font_file, 18) as font_id:  # type: ignore
                    # 添加默认拉丁字符范围
    ...
    def _create_main_window(self):
        """创建主窗口"""
        # 设置主题
        with dpg.theme() as global_theme:  # type: ignore
            with dpg.theme_component(dpg.mvAll):  # type: ignore
                # 颜色
                dpg.add_theme_color(dpg.mvThemeCol_WindowBg, ThemeColors.WINDOW_BG)
                dpg.add_theme_color(dpg.mvThemeCol_ChildBg, ThemeColors.PANEL_BG)
                dpg.add_theme_color(dpg.mvThemeCol_Button, ThemeColors.INPUT_BG)
                dpg.add_theme_color(
                    dpg.mvThemeCol_ButtonHovered, ThemeColors.BUTTON_HOVER
                )
                dpg.add_theme_color(
                    dpg.mvThemeCol_ButtonActive, ThemeColors.BUTTON_ACTIVE
                )
                dpg.add_theme_color(dpg.mvThemeCol_Text, ThemeColors.USER_TEXT)
        ...
        with dpg.window(
            ...
            with dpg.group(horizontal=True):  # type: ignore
                ...
                self.status_text = dpg.add_input_text(
                    default_value="就绪", readonly=True, width=-1
                )
            dpg.add_separator()
            ...
            # 主布局：左右分割（独立滚动）
            with dpg.group(parent=int(content_container), horizontal=True):  # type: ignore
                # 左侧：会话列表 (25%)
                ...
                self.session_list = SessionList(
                    parent=int(left_panel),  # type: ignore
        ...
        self.chat_panel = ChatPanel(
            parent=int(right_panel),  # type: ignore
    ...
    def _load_current_session(self):
        ...
        if session_id and self.session_list:
    ...
    def _on_model_change(self, sender, app_data, user_data):
        ...
        if new_model != old_model:
            ...
            if self.chat_panel:
                self.chat_panel.add_text(
                    f"[提示] 模型已从 {old_model} 切换到 {new_model}\n"
                )
    ...
    def _process_queue(self):
        ...
        self.async_executor.process_queue(
            ...
            complete_callback=lambda: (
                self._update_status("任务完成"),
                self.chat_panel.set_running(False) if self.chat_panel else None,
            ),
    ...
    def _on_pending_commands(self, commands):
        ...
        for index, command in commands:
            display_command = (
                command.display() if hasattr(command, "display") else command
            )
    ...
    def _show_command_confirmation_dialog(self, index: int, command: str):
        ...
        with dpg.window(  # type: ignore
            label="命令确认",
            modal=True,
            id="command_confirmation_dialog",
            pos=[400, 300],
            width=600,
            height=300,
        ):
            ...
            # 命令文本（可折叠）
            with dpg.collapsing_header(label="命令详情", default_open=True):  # type: ignore
            ...
            with dpg.group(horizontal=True):  # type: ignore
                dpg.add_button(
                    label="执行",
                    width=100,
                    callback=lambda: self._on_command_confirm_executed(index, command),
                )
                dpg.add_spacer(width=10)
                dpg.add_button(
                    label="执行并开启自动",
                    width=140,
                    callback=lambda: self._on_command_confirm_execute_with_auto(
                        index, command
                    ),
                )
                dpg.add_spacer(width=10)
                dpg.add_button(
                    label="取消",
                    width=100,
                    callback=lambda: self._on_command_confirm_rejected(index),
                )

            dpg.add_spacer(height=10)
            dpg.add_text(
                "提示：点击 '执行' 将在后台运行命令", color=ThemeColors.HINT_TEXT
            )
...
def main():
    """主函数入口"""
    # 复用 CLI 的参数解析逻辑
    from ..cli import check_llm_connection, parse_args
